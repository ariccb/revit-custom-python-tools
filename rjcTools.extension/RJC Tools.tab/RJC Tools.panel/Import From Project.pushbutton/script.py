...
__doc__='Imports General Notes from a Project File of your choice' \
...
# select file functionality copied from excel loading script
# def select_file():
...
def load_project_in_background():
    def select_file():
        new_file_path = pyrevit.forms.pick_file(file_ext='rvt', \
                                                                init_dir=cal_projects_path, \
                                                                multi_file=False)

        print('the selected file to be loaded in background is: ' + new_file_path)
        return new_file_path
    
    logger.debug('Attempting to load the Revit file')
    final_file_path = select_file()
    # converts into the file path that Revit can read
    modelPath = DB.ModelPathUtils.ConvertUserVisiblePathToModelPath(final_file_path)
    backgroundDoc = app.OpenDocumentFile(modelPath, options)     

    print('The Revit file has been loaded in the background')
...
source_doc, gn_title = load_project_in_background()
